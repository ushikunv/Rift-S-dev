...
public class MeshCut : MonoBehaviour
{
    ...
    public void Cut(Plane cutPlane)
    {
        ...
        foreach(Vector3 position in list1){
            centerPoint1+=position;
        }
        centerPoint1/=list1.Count;
        float volume1=0f;
        for(int i = 0;i<list1.Count;i+=3){
            volume1+=1f/6f*Vector3.Cross(list1[i]-list1[i+1],list1[i+2]-list1[i+1]).magnitude*Vector3.Distance(centerPoint1,(list1[i]+list1[i+1]+list1[i+2])/3);
        }
        UnityEngine.Debug.Log(volume1);
        Vector3 centerPoint2= Vector3.zero;
        foreach(Vector3 position in list2){
            centerPoint2+=position;
        }
        centerPoint2/=list2.Count;
        float volume2=0f;
        for(int i = 0;i<list2.Count;i+=3){
            volume2+=1f/6f*Vector3.Cross(list2[i]-list2[i+1],list2[i+2]-list2[i+1]).magnitude*Vector3.Distance(centerPoint2,(list2[i]+list2[i+1]+list2[i+2])/3);
        }

        //カット後のオブジェクト生成、いろいろといれる
        GameObject obj = new GameObject("cut obj", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider), typeof(Rigidbody), typeof(MeshCut),typeof(OVRGrabbable));
        var mesh = new Mesh();
        ...
        obj.GetComponent<Rigidbody>().mass=GetComponent<Rigidbody>().mass*volume1/(volume1+volume2);
        obj.GetComponent<MeshCut>().skinWidth = skinWidth;
       

        GameObject obj2 = new GameObject("cut obj", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider), typeof(Rigidbody), typeof(MeshCut),typeof(OVRGrabbable));
        var mesh2 = new Mesh();
        ...
        obj2.GetComponent<Rigidbody>().mass=GetComponent<Rigidbody>().mass*volume2/(volume1+volume2);
        obj2.GetComponent<MeshCut>().skinWidth = skinWidth;

   
        GetComponent<OVRGrabbable>().NotifyToGrabber();
            ...
            for (int j = i + 3; j < vertices.Count; j += 3)
            {
                ...
                if (j == vertices.Count - 3)
                {
                    ...
                    if (verticeIndices.Count > 1)
                    {
                        ...
                        //同一平面状のn個の隣接する三角形にはn-1個の等しい直線があるはずが、ない場合(ここまでの処理がうまくいっていない場合)は処理をやめる
                        //ここらへんは検討中　→　新しい処理追加
                        if (sameLineCount != verticeIndices.Count - 1)
                        {
                            //同一平面状にあるが三角形が隣接していない場合（本来同一平面状と判定されるはずの三角形が
